...
void mark_shortest_route_ksp_ecmp(long **k_path, k_paths **routing_table, long paths, long start, long end){

    long i,j;
    long *l_aux;

    if(start == end){
        routing_table[start][end].n_paths = paths;
        routing_table[start][end].paths = malloc(paths * sizeof(long*));
        for(i = 0; i < paths; i++){
            l_aux = k_path[i];
            routing_table[start][end].paths[i] = malloc((l_aux[0] + 1) * sizeof(long));
            routing_table[start][end].paths[i][0] = l_aux[0];
            for(j = 0; j < l_aux[0]; j++){
                routing_table[start][end].paths[i][j+1] = l_aux[j+1];
            }
        }
        ...
        routing_table[start][end].n_paths = paths;
        routing_table[end][start].n_paths = paths;
        routing_table[start][end].paths = malloc(paths * sizeof(long*));
        routing_table[end][start].paths = malloc(paths * sizeof(long*));
        for(i = 0; i < paths; i++){
            l_aux = k_path[i];
            routing_table[start][end].paths[i] = malloc((l_aux[0] + 1) * sizeof(long));
            routing_table[end][start].paths[i] = malloc((l_aux[0] + 1) * sizeof(long));
            routing_table[start][end].paths[i][0] = l_aux[0];
            routing_table[end][start].paths[i][0] = l_aux[0];
            for(j = 0; j < l_aux[0]; j++){
                routing_table[start][end].paths[i][j+1] = l_aux[j+1];
                routing_table[end][start].paths[i][j+1] = l_aux[l_aux[0] - j];
                //printf("%ld - ", l_aux[j+1]);
            }
        }
    }
 ...
 */
void shortest_route_ksp_ecmp(long start, long *length, long *paths, k_paths **routing_table, int K, int ecmp, long switches){
    ...
    for(end = start; end < switches;end++){
        ...
        mark_shortest_route_ksp_ecmp(k_path, routing_table, curr_path + 1, start, end);
        list_reset(&B);
 ...
 * Auxiliar funtion to generate the routing table.
 */
void shortest_route_llksr(long start, long *length, long *paths, k_paths **routing_table, long M, long K, long H, long ths, long switches){
...
void sp_paths(graph_t *rg, long switches, k_paths **routing_table){
    ...
    k_path = malloc(routing_params[0] * sizeof(long*)); 

    list_initialize(&B, sizeof(long**));

    for(i = 0; i < switches; i++) {
        find_shortest_path(random_graph, switches, i, path, paths_distances);
        shortest_route_ksp_ecmp(i, paths_distances, path, routing_table, 1, 0, switches);
    }
    free(path);
...
void ksp_paths(graph_t *rg, long switches, k_paths **routing_table, long ecmp){
    ...
    for(i = 0; i < switches; i++) {
        find_shortest_path(random_graph, switches, i, path, paths_distances);
        shortest_route_ksp_ecmp(i, paths_distances, path, routing_table, k, ecmp, switches);
    }
    free(path);
    free(paths_distances);
    free(root_path);
...
void llksr_paths(graph_t *rg, long switches, k_paths **routing_table, long r, long p){
    ...
    for(i = 0; i < switches; i++) {
        find_shortest_path(random_graph, switches, i, path, paths_distances);
        shortest_route_llksr(i, paths_distances, path, routing_table, M, K, H, ths, switches);
    }
    free(path);
    free(paths_distances);
    free(root_path);
...
void generate_routing_table(long ports_switches, long ports_servers)
{
    ...
    for(i = 0; i < switches; i++) {
        ...
        for(j = 0; j < switches; j++) {
            routing_table[i][j].n_paths = 1; 
            routing_table[i][j].last_used = malloc(size * sizeof(long));; 
            for(k = 0; k < size; k++){
                routing_table[i][j].last_used[k] = 0; 
            }
        }
    }
    ...
    switch(routing) {
        case JELLYFISH_SHORTEST_PATH_ROUTING:
            sp_paths(random_graph, switches, routing_table);
            break;
        case JELLYFISH_K_SHORTEST_PATHS_ROUTING:
            ksp_paths(random_graph, switches, routing_table, 0);
            break;
        case JELLYFISH_ECMP_ROUTING:
            ksp_paths(random_graph, switches, routing_table, 1);
            break;
        case JELLYFISH_LLSKR_ROUTING:
            llksr_paths(random_graph, switches, routing_table, ports_switches, ports_servers);
            break;
        default:
            printf("Unknown routing.\n");
            exit(-1);
...
void destroy_routing_table()
{
    ...
    switch(routing) {
        ...
        case JELLYFISH_LLSKR_ROUTING:
            for(i = 0; i < switches; i++) {
                for(j = 0; j < switches; j++) {
                    for(k = 0; k < routing_table[i][j].n_paths;k++){
                        free(routing_table[i][j].paths[k]);
                    }
                    free(routing_table[i][j].paths);
                    free(routing_table[i][j].last_used);
                }
    ...
    switch(routing) {
        ...
        case JELLYFISH_LLSKR_ROUTING:
            for(i = 0; i < switches; i++) {
                for(j = 0; j < switches; j++) {
                    for(k = 0; k < routing_table[i][j].n_paths; k++) {
                        printf(" %ld ",routing_table[i][j].paths[k][0]);
                    }
                }
                printf("\n");
            }
