...
long param_a;	///< parameter a (= alpha), the number of leaders adjacent to a switch in HCN(n,h)
long param_b;	///< parameter b (= beta), the number of followers adjacent to a switch in HCN(n,h)
long param_h;	///< parameter h, depth of recursion of HCN(n,h)
long param_n; ///< radix of switch
long param_c; ///< value of c (= gamma) in BCN(a,b,h,c)
long param_r;///< connection type, 0 or 1
long param_t;///< tightness parameter.
...
long servers; 	///< the total number of servers (= N): leaders + followers
long switches;	///< the total number of switches: a^h
long ports;///< total number of ports USED in network:
...
///and not a struct
list_t route_list; ///< list that proxy_routing will append to on the top level
static long link[2]; ///<store the output of get_link

long **hcnlabels; ///< coordinate labels for copies of HCN(a,h)
long **uid_i;///< uid_i for hcn leaders (functions map to these for other nodes)
struct Buv{
	long hcn;
	long u;
	long v;
