...
long N_sr, N_sw, N, port, col, turn_back, diameter;
...
long rout_alg, dir;	// used for DPillar network
...
long all_path[100][50];
long min_route_hist[10]= {0};
long ver_index, hor_index;
...
long DPillarSP(long src, long dst, long dir)
{
	...
	if(src != dst) {
		...
		while(crn!=dst) {
			...
			if ( turn_back==1 && (abs(dst-crn))%(long)pow(port/2.0, col)==0 ) {
