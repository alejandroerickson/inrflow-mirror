...
FILE *stats_file, *hist_file, *tex_file;
char filename_params[100];
char stats_filename[100], hist_filename[100], prefix_filename[100], tex_filename[100];
...
//#define PRINT_CONGESTED_FLOWS
  ...
  sprintf(prefix_filename,"%s_%s_%s_%s_fr%.2f_seed%ld.%s",
					...
					r_seed,
					time_started);
	snprintf(stats_filename,100,"%s.dat",prefix_filename);
  ...
  add_key_value(&stats_head,
	...
	///< This is safe without implementing it for topologies that do not have
	///routing algorithm choices, since the NULL function will never be called.
	for(i=0; i< routing_nparam; i++){
		get_routing_param_tokens(i);
		add_key_value(
	...
	snprintf(hist_filename,100,"%s_hist.dat",prefix_filename);
	...
	long exit_val;
	snprintf(tex_filename,100,"%s.tex",prefix_filename);
	// delete the \end{document}
	snprintf(sys_command,1000,
					 "sed '$ d' pretty_data.tex > %s",tex_filename);
	//sprintf(sys_command,"cp pretty_data.tex %s.tex",tex_filename);
	exit_val=system(sys_command);
        if(exit_val == 0){
	tex_file = fopen(tex_filename,"a");
	if(tex_file==NULL){
		printf("ERROR: Could not open tex file.\n");
		exit(-1);
	}
	for (i = 0; i < topo_nhists; i++) {
		fprintf(tex_file,
						...
						"   grid=major,\n"
						"   ]\n"
						"   \\addplot table[x=%ci,y=%c]{\\histogramtable};\n"
						"\\end{axis}\n"
						"\\end{tikzpicture}\n\n\n"
						,topo_hists_doc[i],topo_hists_prefix[i],topo_hists_prefix[i]);
	}
	fprintf(tex_file,"\\end{document}");
	fclose(tex_file);

	printf("Compile latex with the following command.\npdflatex"
				 "  \"\\newcommand\\thetables{\\pgfplotstabletranspose"
				 "\\statstable{%s} \\pgfplotstableread{%s}\\histogramta"
				 "ble}\\input{%s}\"\n",stats_filename,hist_filename,tex_filename);
        }
#ifdef DEBUG
...
long create_latex_output()
{
	...
	if(compile_latex) {
		...
		sprintf(sys_command,"pdflatex -interaction nonstopmode \"\\newcommand"
						"\\thetables{\\pgfplotstabletranspose\\statstable{%s} "
						"\\pgfplotstableread{%s}\\histogramtable}\\input{%s}\""
						" > /dev/null 2>&1"
						,stats_filename,hist_filename,tex_filename);
		exit_code = system(sys_command);
		// Pitfalls in latex compilation: If pdflatex encounters bad characters it will try and
		// continue because of nonstopmode, but will ultimately return a non-zero exit value.
		// Check the exit value and try running the command manually to debug.  Maybe your
		// topology has a histogram with bad "documentation", which gets added to the tex file.
		if(exit_code==0) {
