...
long param_k;	///< parameter k of the topology, number of levels of the ficonn (-1)
long param_n;	///< parameter n of the topology, switch radix

long *n_pow;	///< an array with the n^i, useful for doing some calculations.
long *two_pow;	///< an array with the 2^i, useful for doing some calculations.
long *fic_lvl;	///< number of ficonn_k-1 to form a ficonn_k
long *fic_size;	///< number of servers in a ficonn_k
long *free_ports;///< number of free ports in a ficonn_k

long switches;	///< The total number of switches : 2(k+2)*((n/4)^(2^k))
long servers; 	///< The total number of servers : switches/n
long ports;		///< The total number of links

long *src; ///< The coordinates for the current
long *dst; ///< The coordinates for the destination
...
tuple_t connection_ficonn(long node, long port)
{
	...
	if (network[node].port[port].neighbour.node!=-1) // if this node is connected already
		...
		if (port==0) { //ficonn0 connections
			...
			/*			printf("0----> %5d [",node);
						for(i=1;i<=param_k;i++)
							printf("%3d,",(node/fic_size[i-1])%fic_lvl[i]);
						printf("%3d] - lvl 0 ", node%fic_size[0]);
						printf("`----> switch %ld.%ld\n",res.node,res.port);
			*/
		} else { // level>0;
			...
			if (lvl>param_k) { // if the break is not reached then this is a free port
				...
				*/
				...
				// Coordinates are calculated here.
				/*				printf("+----> %5d [",node);
								for(i=1;i<=param_k;i++)
									printf("%3d,",(node/fic_size[i-1])%fic_lvl[i]);
								printf("%3d] - lvl %ld - i1 %ld, j1 %ld\n", node%fic_size[0], lvl, i1, j1);
				*/
				...
				/*				printf("`----> %5d [",pos);
								for(i=1;i<=param_k;i++)
									printf("%3d,",(pos/fic_size[i-1])%fic_lvl[i]);
								printf("%3d] - lvl %ld - i2 %ld, j2 %ld\n\n", pos%fic_size[0], lvl, i2, j2);
				*/
...
{
	//printf("\nattempting routing from %ld to %ld\n",src,dst);
...
void finish_route_ficonn()
{
	if (last_route->first!=NULL) {
		printf("Message arrived to its destination, but there is still some ports in the route\nDumping route:\n");
		while (last_route->first!=NULL)
			printf("%ld; ",get_next(last_route)); // dumping and freeing elements
	}
	free(last_route);
	// rfree++;
	// printf("%ld, %ld\t",ralloc,rfree);
}
